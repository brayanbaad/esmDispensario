<?php
class RegistroPacienteModel extends Query{
    ...
    public function getVerificar($item,$identificacion,$id){
        if ($id > 0) {
            $sql = "SELECT * FROM pacientes WHERE $item = '$identificacion' AND id !=$id  ";
        } else {
            $sql = "SELECT * FROM pacientes WHERE $item = '$identificacion'";
        }
