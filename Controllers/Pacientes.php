...
class Pacientes extends Controller{
    ...
    public function registrar(){
        ...
        if (empty($tipoIdentificacion)|| empty($identificacion) || empty($apellidos) || empty($nombres) 
            ...
            if ($id_paciente==""){
                $verificarPaciente= $this->models->getVerificar('identificacion', $identificacion,0);
                print_r($verificarPaciente);
                exit();
                // if (empty($verificarPaciente)) {
                //     $data = $this->model->registrarPaciente($fechaRuta,$tipoIdentificacion,$identificacion,$apellidos,$nombres,$fechaNacimiento,$edad,$telefono,$direccion,$nivelEducativo,$ocupacion,
